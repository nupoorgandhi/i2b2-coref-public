...
class NonContextModel(CorefConceptModel):
    def __init__(self, config, args):
        self.args = args
        self.probe_rank = args['probe']['maximum_rank']
        self.word_pair_dims = (0, 1)
        self.meta_data = []
        super(NonContextModel, self).__init__(config)
    ...
    def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends, k,
        ...
        if self.config["model_heads"]:
            ...
            transformed_head_attn_reps = util.linear(head_attn_reps,
                                                     [k, self.probe_rank])  # [num_candidates, probe_rank]

            if self.config['change_span']:

                if self.config['concat']:
                    head_attn_reps = tf.concat([head_attn_reps, transformed_head_attn_reps], 1)
                else:
                    head_attn_reps = transformed_head_attn_reps

            # else:
            #     transformed_head_attn_reps = head_attn_reps
            sample_num = 500
            ave_projection_loss = 0.0
            for i in range(100):
                idxs = tf.range(tf.shape(transformed_head_attn_reps)[0])
                ridxs = tf.random.shuffle(idxs)[:sample_num]
                transformed_head_attn_reps_tr = tf.gather(transformed_head_attn_reps, ridxs)
                ...
                # PROJECTION LOSS
                distance_pred = util.tf_cosine_distance(transformed_head_attn_reps_tr)
                # distance_pred = self.squared_dist_predictions(transformed_head_attn_reps_tr)
                # compute the labels
                distance_labels = self.labels(transformed_head_attn_reps_tr, candidate_cluster_ids_tr,
        ...
        return span_emb, ave_projection_loss, head_attn_reps  # [k, emb]
    ...
    def labels(self, span_emb, cluster_ids, concept_ids, umls_ids, num_candidates):
        ...
        # cosine_distances = util.tf_cosine_distance(span_emb)


        # concept_distances = 1 - (concept_ids[:, None] - concept_ids == 0).astype(int)
        # inv_mask = (concept_ids == 0) | (concept_ids == 5)
        # concept_distances[inv_mask] = 1
        # np.fill_diagonal(concept_distances, 0)
        # # TODO add interpolation of the cosine distance between the spans
        # # span_emb = observation[-1]
        # cosine_dist = cosine_distances(span_emb)
        # # print(pprefix, 'cosine_distances', cosine_distances.shape)
        # # print(pprefix, 'coref_distances', span_emb.shape)

        concept_weight = self.args['dataset']['concept_weight']

        emb_weight = self.args['dataset']['emb_weight']
        ...
        # distances = (emb_weight * cosine_distances) + (coref_weight * coref_distances) + (concept_weight * concept_distances)
    ...
    def squared_dist_predictions(self, transformed):
        ...
        #
        # seqlen, rank = transformed.get_shape()  # seqlen is number of spans in doc
        # transformed = tf.expand_dims(transformed, axis=2)  # [batchlen, seqlen, 1, rank]
        # # transformed = transformed.expand(-1, -1, seqlen, -1) #[batchlen, seqlen, seqlen, rank]
        # transformed = tf.broadcast_to(transformed, shape=[1, seqlen, seqlen, rank])
        # # transposed = transformed.transpose(1,2) #[batchlen, seqlen, seqlen, rank]
        # transposed = tf.transpose(transformed, perm=[0, 2, 1, 3])
        # # print(pprefix, 'transformed', transformed.shape, 'transposed', transposed.shape)
        # diffs = transformed - transposed
        #
        # # squared_diffs = diffs.pow(2)
        # squared_diffs = tf.pow(diffs, 2)
        # # squared_distances = torch.sum(squared_diffs, -1)
        # squared_distances = tf.reduce_sum(squared_diffs)
    ...
    #                              gold_ends, cluster_ids, sentence_map):
        ...
        candidate_span_emb, projection_loss, candidate_attn_reps = self.get_span_emb(mention_doc, mention_doc, candidate_starts,
        ...
        joint_loss = coref_loss+ self.config["projection_loss_weight"]*projection_loss

        return [candidate_starts, candidate_ends, candidate_mention_scores, candidate_span_emb, top_span_starts,
            top_span_ends, top_antecedents, top_antecedent_scores, projection_loss], joint_loss
    ...
    def evaluate(self, session, model_id, global_step=None, official_stdout=False, keys=None, eval_mode=False):
        ...
        # print('incompat_distances', incompat_distances)
