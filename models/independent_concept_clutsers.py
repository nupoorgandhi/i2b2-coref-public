...
import debug_util
import span_util
# from models.independent import CorefModel
from minimize import CONCEPT_CODE_DICT

class CorefConceptModel(object):
    def __init__(self, config):
        ...
        input_props.append((tf.int32, [None]))  # Concept starts.
        input_props.append((tf.int32, [None]))  # Concept ends.
        input_props.append((tf.int32, [None]))  # Concept cluster ids
        input_props.append((tf.int32, [None]))  # umls starts.
        input_props.append((tf.int32, [None]))  # umls ends.
        input_props.append((tf.int32, [None]))  # umls cluster ids
        ...
        queue = tf.PaddingFIFOQueue(capacity=16, dtypes=dtypes, shapes=shapes)
        self.enqueue_op = queue.enqueue(self.queue_input_tensors)
        self.input_tensors = queue.dequeue()

        self.predictions, self.loss, self.to_print = self.get_predictions_and_loss(*self.input_tensors)
        # bert stuff
        ...
        if 'freeze_bert_layer' in self.config:
          freeze_bert_layer = self.config['freeze_bert_layer']
        else:
          freeze_bert_layer = -1
        self.train_op = optimization.create_custom_optimizer(tvars,
                          ...
                          num_train_steps, num_warmup_steps, False, self.global_step, freeze=freeze_bert_layer,
    ...
    # example at the document level
    def tensorize_example(self, example, is_training):
        pprefix = '[tensorize_example]'
        clusters = example["clusters"]
        # print(pprefix, 'util.flatten(clusters)', clusters, 'after\n', util.flatten(clusters))
        ...
        # concept_clusters = util.flatten_concepts({})
        concept_clusters = util.flatten_concepts(example["concept_clusters"])
        gold_concepts = sorted(tuple(m) for m in util.flatten(concept_clusters))
        gold_concept_map = {m: i for i, m in enumerate(gold_concepts)}
        concept_cluster_ids = np.zeros(len(gold_concepts))
        for cluster_id, cluster in enumerate(concept_clusters):
            for mention in cluster:
                concept_cluster_ids[gold_concept_map[tuple(mention)]] = cluster_id + 1#int(CONCEPT_CODE_DICT[cluster_id]) + 1

        umls_clusters = util.flatten_umls(example['umls_clusters'], self.config['umls_key'])
        gold_umls = sorted(tuple(m) for m in util.flatten(umls_clusters))
        gold_umls_map = {m: i for i, m in enumerate(gold_umls)}
        umls_cluster_ids = np.zeros(len(gold_umls))
        for cluster_id, cluster in enumerate(umls_clusters):
            for mention in cluster:
                umls_cluster_ids[gold_umls_map[tuple(mention)]] = cluster_id + 1



        # print(pprefix, 'concept_cluster_ids', concept_cluster_ids)
        sentences = example["sentences"]
        # print(pprefix, 'sentences',  example["sentences"])
        num_words = sum(len(s) for s in sentences)
        # print(pprefix, 'speakers',example["speakers"])
        ...
        # print(pprefix, 'num_words', num_words)
        # print(pprefix, 'np.sum(input_mask)', np.sum(input_mask), input_mask.shape)
        # for i,x in enumerate(input_mask):
        #   print(i,'x in input_maks:', len(x), max_sentence_length)
        # print(pprefix, '(num_words, np.sum(input_mask))', (num_words, np.sum(input_mask)))
        ...
        # print('gold_concepts', gold_concepts)
        gold_concept_starts, gold_concept_ends = self.tensorize_mentions(gold_concepts)
        gold_umls_starts, gold_umls_ends = self.tensorize_mentions(gold_umls)

        example_tensors = (
        input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids,
        gold_concept_starts, gold_concept_ends, concept_cluster_ids,
        gold_umls_starts, gold_umls_ends, umls_cluster_ids,
        sentence_map)
    ...
    def truncate_example(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends,
                         cluster_ids, concept_starts, concept_ends, concept_cluster_ids, umls_starts, umls_ends,
                         umls_cluster_ids,sentence_map, sentence_offset=None):
        ...
        concept_spans = np.logical_and(concept_ends >= word_offset, concept_starts < word_offset + num_words)
        concept_starts = concept_starts[concept_spans] - word_offset
        concept_ends = concept_ends[concept_spans] - word_offset
        concept_cluster_ids = concept_cluster_ids[concept_spans]

        umls_spans = np.logical_and(umls_ends >= word_offset, umls_starts < word_offset + num_words)
        umls_starts = umls_starts[umls_spans] - word_offset
        umls_ends = umls_ends[umls_spans] - word_offset
        umls_cluster_ids = umls_cluster_ids[umls_spans]
        # return input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, \
        #    sentence_map
        return input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, \
               concept_starts, concept_ends, concept_cluster_ids, umls_starts, umls_ends, umls_cluster_ids,\
               sentence_map
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts,
                                 ...
                                 umls_starts, umls_ends, umls_cluster_ids,sentence_map):
        ...
        candidate_concept_cluster_ids = self.get_candidate_labels(candidate_starts, candidate_ends, concept_starts, concept_ends,
                                                          concept_cluster_ids)  # [num_candidates]

        candidate_span_emb = self.get_span_emb(mention_doc, mention_doc, candidate_starts,
                                               candidate_ends, candidate_concept_cluster_ids)  # [num_candidates, emb]
        ...
        print(pprefix, 'top_span_indices', top_span_indices.get_shape())
        # exit(0)
        # tf.print("tensors:", top_span_indices, {2: top_span_indices * 2}, output_stream=sys.stdout)
        ...
        return [candidate_starts, candidate_ends, candidate_mention_scores, candidate_span_emb, top_span_starts,
                top_span_ends, top_antecedents, top_antecedent_scores], loss, 0.0


    # def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends):
    def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends, concept_cluster_ids):
        span_emb_list = []
        ...
        if self.config["use_features"]:
          ...
          if self.config["use_i2b2_concepts"]:
              concept_cluster_emb = tf.gather(
                  tf.get_variable("concept_embeddings", [self.config["num_i2b2_concepts"], self.config["feature_size"]],
                                  initializer=tf.truncated_normal_initializer(stddev=0.02)),
                  concept_cluster_ids)  # [k, emb]
              span_emb_list.append(concept_cluster_emb)
    ...
    def evaluate(self, session, model_id, global_step=None, official_stdout=False, keys=None, eval_mode=False):
        ...
        for example_num, (tensorized_example, example) in enumerate(self.eval_data):
            ...
            loss, (
                candidate_starts, candidate_ends, candidate_mention_scores, candidate_span_emb, top_span_starts,
                top_span_ends, top_antecedents,
                top_antecedent_scores) = session.run([self.loss, self.predictions], feed_dict=feed_dict)
    ...
    def start_enqueue_thread(self, session):
        if len(str(self.config['sample_id'])) > 1:
            train_examples = debug_util.get_sample_jsonlines(self.config['sample_id'], self.config["train_path"])
        else:
    ...
    def get_width_scores(self, doc, starts, ends):
        ...
        with tf.variable_scope('span_width'):
            span_width_emb = tf.gather(tf.get_variable("start_width_embeddings", [self.config["max_span_width"], hidden],
                                                       initializer=tf.truncated_normal_initializer(stddev=0.02)),
                                       distance)  # [W, emb]
    ...
    def softmax_loss(self, antecedent_scores, antecedent_labels):
        ...
        # print(pprefix, 'antecedent_scores', type(antecedent_scores), antecedent_scores.shape, type(antecedent_labels), antecedent_labels.shape)
    ...
    def load_eval_data(self, eval_mode=False):
        if self.eval_data is None:
            def load_line(line):
                example = json.loads(line)
                return self.tensorize_example(example, is_training=False), example

            if not eval_mode and len(str(self.config['sample_id'])) > 1:
                lines = debug_util.get_sample_jsonlines(self.config['sample_id'], self.config["dev_path"])
                self.eval_data = [(self.tensorize_example(x, is_training=False), x) for x in lines]
            else:
                if eval_mode:
                    with open(self.config["eval_path"]) as f:
                        self.eval_data = [load_line(l) for l in f.readlines()]
                else:
                    with open(self.config["dev_path"]) as f:
                        self.eval_data = [load_line(l) for l in f.readlines()]

                # if self.config['truncate'] > 0:
                #   self.eval_data = self.eval_data[:self.config['truncate']]
