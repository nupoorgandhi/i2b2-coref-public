...
class CorefModel(object):
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ...
    candidate_span_emb, candidate_head_attn_reps = self.get_span_emb(mention_doc, mention_doc, candidate_starts, candidate_ends) # [num_candidates, emb]
    # loop through the starts and ends, produce the
    ...
    return [candidate_starts, candidate_ends, candidate_mention_scores, candidate_head_attn_reps, top_span_starts, top_span_ends, top_antecedents, top_antecedent_scores], loss
  ...
  def get_span_emb(self, head_emb, context_outputs, span_starts, span_ends):
    ...
    if self.config["project"]:
      pass
      # TODO: load the projection matrix B
      # check sizes
      # project span_emb into latent space using B
      # concat again to produce a new span_emb
  ...
  def evaluate(self, session, model_id, global_step=None, official_stdout=False, keys=None, eval_mode=False):
    ...
    for example_num, (tensorized_example, example) in enumerate(self.eval_data):
      ...
      loss, (
        candidate_starts, candidate_ends, candidate_mention_scores, candidate_span_emb, top_span_starts, top_span_ends, top_antecedents,
    ...
    debug_util.save_pickle({'projections_flattened': projections_flattened, 'proj_name': span_pair_names,
                            'x_emb_flattened':x_emb_flattened},
                           'baseline_model_span_pairs_proj_flattened.pickle')
