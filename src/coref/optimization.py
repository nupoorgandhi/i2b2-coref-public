...
def create_custom_optimizer(tvars, loss, bert_init_lr, task_init_lr, num_train_steps, num_warmup_steps,
                            use_tpu, global_step=None, freeze=-1, task_opt='adam', eps=1e-6):
  ...
  for var in tvars:
    if var.name.startswith('bert'):
        can_optimize = False
        # check that layer at least as big as "freeze"
