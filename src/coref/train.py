...
if __name__ == "__main__":
  ...
  # with tf.Session() as session:
    ...
    # print('(just loaded model)gpu memory used', session.run(tf.contrib.memory_stats.MaxBytesInUse()))
    ...
    while True:
      ...
      # train_op is the update step
      # print('tf_loss', tf_loss)
      # print('to_print', to_print)
      # to_print = tf_loss[1]
      # tensors_to_run = [x[0] for x in to_print]
      # tensor_names = [x[1] for x in to_print]
      #
      # vals = sess.run(tensors_to_run)
      # for i, name in enumerate(tensor_names):
      #   print(name + '\n', vals[i])
      # print('tf_loss', tf_loss, tf_loss[1].shape, tf_loss[2].shape, tf_loss[3].shape)
      # print('(ran session)gpu memory used', session.run(tf.contrib.memory_stats.MaxBytesInUse()))


      accumulated_loss += tf_loss
      # accumulated_to_print.append(np.array(to_print))
      ...
      if tf_global_step % report_frequency == 0:
        ...
        # print('to_print',np.mean(accumulated_to_print, axis=1))
        accumulated_to_print = []


      if tf_global_step  > 0 and tf_global_step % eval_frequency == 0:
        saver.save(session, os.path.join(log_dir, "model"), global_step=tf_global_step)
        eval_summary, eval_f1 = model.evaluate(session,sys.argv[1], tf_global_step)
        ...
        if tf_global_step > 61500:
          print('[main] reached 61500')
          break
        if (tf_global_step > max_steps or tf_global_step > 70200) and 'tuneTrue' not in sys.argv[1]:
